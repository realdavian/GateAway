...
final class AppCoordinator: AppCoordinatorProtocol {
  ...
  func getTopServers(forCountry country: String) -> [VPNServer] {
    ...
    return selectionService.topServers(
      from: servers, country: country, limit: preferences.topKPerCountry)
  }
  ...
  func connectToBestServer() async throws {
    ...
    guard let bestServer = await selectionService.selectBestServerAsync(from: servers) else {
      throw NSError(
        domain: "AppCoordinator", code: 1,
        userInfo: [NSLocalizedDescriptionKey: "No servers available"])
    }
