...
struct ConnectionAttempt: Codable {
  ...
  init(
    serverID: String, success: Bool, connectionTime: TimeInterval?, retryCount: Int,
...
protocol TelemetryProtocol {
  ...
  func getOverallStats() -> (
    totalAttempts: Int, successCount: Int, avgConnectionTime: TimeInterval
  )?
}
...
class ConnectionTelemetry: ObservableObject, TelemetryProtocol {
  ...
  private let maxHistoryDays = Constants.Limits.telemetryHistoryDays
  ...
  private func recalculateStats() {
    ...
    for (serverID, attempts) in groupedAttempts {
      ...
      let avgTime =
        successfulTimes.isEmpty ? 0 : successfulTimes.reduce(0, +) / Double(successfulTimes.count)
