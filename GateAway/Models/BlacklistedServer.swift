...
struct BlacklistedServer: Codable, Identifiable {
  ...
  init(
    id: String, hostname: String, country: String, countryShort: String, reason: String,
    blacklistedAt: Date, expiresAt: Date?
  ) {
    self.id = id
  ...
  init(from decoder: Decoder) throws {
    ...
    countryShort =
      try container.decodeIfPresent(String.self, forKey: .countryShort)
      ?? String(country.prefix(2)).uppercased()
