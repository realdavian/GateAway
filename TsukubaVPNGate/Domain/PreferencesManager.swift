...
final class PreferencesManager: PreferencesManagerProtocol {
    ...
    func loadPreferences() -> UserPreferences {
        let autoReconnect = defaults.object(forKey: Keys.autoReconnectOnDrop) as? Bool ?? UserPreferences.default.autoReconnectOnDrop
        let scopeRaw = defaults.string(forKey: Keys.reconnectScope) ?? UserPreferences.default.reconnectScope.rawValue
        let scope = UserPreferences.ReconnectScope(rawValue: scopeRaw) ?? .sameCountry
        let topK = defaults.object(forKey: Keys.topKPerCountry) as? Int ?? UserPreferences.default.topKPerCountry
        let providerRaw = defaults.string(forKey: Keys.vpnProvider) ?? UserPreferences.default.vpnProvider.rawValue
