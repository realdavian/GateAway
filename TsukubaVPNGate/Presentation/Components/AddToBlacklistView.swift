...
struct AddToBlacklistView: View {
    ...
    init(preselectedServer: VPNServer? = nil, onAdd: @escaping (VPNServer, String, BlacklistExpiry) -> Void) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Picker("Server", selection: $selectedServerId) {
                                ...
                                ForEach(serverStore.servers.sorted { $0.countryLong < $1.countryLong }) { server in
                                    Text("\(flagEmoji(for: server.countryShort)) \(server.countryLong) - \(server.ip)")
                                        .tag(server.ip)
                                }
                            }
                            .pickerStyle(.menu)
